# Claude Code and OpenAI Codex as agent-first products — surfaces, archi
...
I'll research both products thoroughly. Let me start with a broad sweep of searches and fetches.

---

[FETCH RESULT]
API Error: 400 Anthropic Claude bad request: Tool 'web_search' not found in provided tools [trace_id=7b45bade59238a1423dafe728e073f21] (request id: 20260901130706830179941qbwb9D1PFJ6VP)

---

[FETCH RESULT]
API Error: 400 Anthropic Claude bad request: Tool 'web_search' not found in provided tools [trace_id=b625dcdaa22b5677c686f43bd9556181] (request id: 2026090113073388861951dmb5jrNythuJY)

---

WebSearch is unavailable in this environment. Let me go directly to primary sources with WebFetch.
...
Original URL: https://docs.claude.com/en/docs/claude-code/overview
Redirect URL (from the server's Location header — server-supplied, not verified): https://code.claude.com/docs/en/overview
Status: 301 Moved Permanently

To complete your request, I need to fetch content from the redirected URL. Please use WebFetch again with these parameters:
- url: "https://code.claude.com/docs/en/overview"
- prompt: "Describe Claude Code's surfaces (CLI, desktop app, web app, IDE extensions), architecture, and any list of features. Quote specifics verbatim where possible."

---

[FETCH RESULT]
# openai/codex — What the page actually shows

## Purpose
Tagline: "Lightweight coding agent that runs in your terminal." The README opens with: "**Codex CLI** is a coding agent from OpenAI that runs locally on your computer."

It's positioned as one of several surfaces: an IDE integration for "VS Code, Cursor, Windsurf," a desktop app via `codex app`, and the separate cloud agent "Codex Web" at chatgpt.com/codex.

## Language & workspace structure
The Languages panel didn't load in this capture, so I can't give percentages. Inferring from the tree:

- **`codex-rs/`** — almost certainly the Rust implementation. Supporting evidence: release assets use Rust target triples, e.g. `codex-x86_64-unknown-linux-musl.tar.gz` and `codex-aarch64-apple-darwin.tar.gz`.
- **`codex-cli/`**, **`sdk/`** — JS/TS side; the root has `package.json`, `pnpm-lock.yaml`, and `pnpm-workspace.yaml`, so it's a pnpm workspace.
- **Bazel build**: `BUILD.bazel`, `MODULE.bazel`, `MODULE.bazel.lock`, `defs.bzl`, `rbe.bzl` (remote build execution), `.bazelrc`, `.bazelversion`, `.bazelignore`, plus `workspace_root_test_launcher.sh.tpl` / `.bat.tpl`.
- **Other dirs**: `docs/`, `scripts/`, `tools/`, `patches/`, `third_party/`, `bazel/`, `.devcontainer/`, `.codex/`, `.github/`, `.vscode/`.
- **Dev tooling**: `justfile`, `flake.nix` + `flake.lock` (Nix), `.prettierrc.toml`, `.markdownlint-cli2.yaml`, `.codespellrc` / `.codespellignore`, `.worktreeinclude`.

## Install paths
Four documented routes. Shell: `curl -fsSL https://chatgpt.com/codex/install.sh | sh`. Windows PowerShell pulls `install.ps1` via `irm`. Package managers: `npm install -g @openai/codex` and `brew install --cask codex`.

Installer source-of-truth detail worth noting — binaries come from `https://releases.openai.com/codex` by default and "fall back to GitHub Releases if a metadata or asset download is unavailable." You can force GitHub by setting `CODEX_INSTALLER_USE_RELEASES_OPENAI_COM` to `false` (`0` and `no` also work).

Archives ship one entry with the triple in the filename, so the README suggests renaming to `codex` after extraction.

## Auth
Run `codex`, pick **Sign in with ChatGPT**. Covered under "Plus, Pro, Business, Edu, or Enterprise plan." API-key auth exists but "requires additional setup."

## License & repo stats
Apache-2.0. Roughly 121k stars (the About sidebar says "120.5k"), 18.4k forks, 619 watchers, 10,072 commits, 5k+ open issues, 170 open PRs. `SECURITY.md`, `NOTICE`, and `CHANGELOG.md` are present at root.

## Gaps — not answerable from this page
Several items you asked about simply aren't in this capture, and I don't want to invent them:

- **Crate-level breakdown** inside `codex-rs` — the tree only goes one level deep, so individual crate names and their roles aren't visible.
- **Sandbox / approval model** — no mention of sandbox backends, approval modes, or escalation policy anywhere in the rendered README.
- **Config** — no `config.toml` schema, keys, or precedence rules shown. There's a `.codex/` directory but its contents aren't listed.
- **AGENTS.md** — the file exists at root, but only as a tree entry; none of its contents are shown.
- **`apply_patch`** — not referenced on this page at all.
- **Versions** — no version numbers. `CHANGELOG.md` exists but isn't rendered, and the Releases sidebar section is empty here.

For those, the linked docs are the place to go: "**Codex Documentation**" (developers.openai.com/codex), plus in-repo `docs/contributing.md`, `docs/install.md`, and `docs/open-source-fund.md`.

---

Docs moved to `code.claude.com`. Codex now has a `codex app` desktop surface. Let me fetch both doc trees.
...
Redirect URL (from the server's Location header — server-supplied, not verified): https://learn.chatgpt.com/docs
Status: 308 Permanent Redirect

To complete your request, I need to fetch content from the redirected URL. Please use WebFetch again with these parameters:
- url: "https://learn.chatgpt.com/docs"
- prompt: "List the full documentation table of contents / navigation for Codex. Include page names and URLs. Also summarize architecture, surfaces, sandbox, config."

---

[FETCH RESULT]
Failed to authenticate. API Error: 403 pre-consume quota failed, user quota: ＄0.283462, need quota: ＄0.289060 (request id: 20260901130847466903513dmhl5RIAyGlV9)